...
def mag2flux(eff_m):
    """Return the flux (erg·cm⁻²·s⁻¹·Å⁻¹) for a given effective magnitude \
    (dimensionless)."""
    return 10**((m_0 - eff_m)/2.5)


def flux2mag(flux):
    """Return the effective magnitude (dimensionless) for a given flux \
    (erg·cm⁻²·s⁻¹·Å⁻¹)."""
...
def z2a(z):
    """Return the expansion factor (a, dimensionless),  for a given redshift \
    (z, dimensionless)."""
    return 1 / (1 + z)


def a2H(a, Om_cc, R = R_0, Om_M0 = 'flat'):
    """Return the Hubble Parameter (H, s⁻¹) for a given expansion factor (a, \
    dimensionless).
...
def z2eta(z, Om_cc, R = R_0, Om_M0 = 'flat'):
    """Return the comoving coordinate (η, dimensionless) for a given redshift \
    (z, dimensionless).
...
def varparam_err_chisq(model_func, bestfit_param, func_args, chisq_min,
                       observed_data, error_in_data):
    """Return the uncertainty of the given varied parameter for χ² \
    (dimensionless) best fit.
...
def calc_min_chisq(model_func, var_param, func_args, observed_data,
                   error_in_data, return_stats = False):
    """Return reduced minimised χ² (dimensionless) (and stats if 'return_stats' \
...
def strarray_add_column(strarray, column_data, column_header, column_dtype,
                        print_array = False):
...
def plot_graph(x_axis, y_axis, **kwargs):
    ...
    if 'plot_line' in kwargs:
        ...
        if trend_dimension == 1: #Given as: kwargs['plot_line'] = (x_axis, y_axis, kwargs)
            ...
            raise ValueError("plot_line MUST be given as: a tuple, (x_array, \
                              y_array, kwargs); or a list of tuples, \
...
def show_graphs(Graph1, *graphs, **kwargs):
    ...
    axes = [pyplot.subplot(subplot)] #Create subplot for first graph.
    plot_graph(x_axis, y_axis, **gr_kwargs) #Plot first graph.

    for n, graph in enumerate(graphs): #Cycle through other graphs.
        if len(graph) < 3: #Raise error if not enough parameters given.
            raise ValueError("Each graph must containt at least (subplot, \
                              x_axis, y_axis)")
        ...
        if type(subplot) == dict:
            try:
                ...
                raise ValueError("You mus put a position ('pos') kwarg in the subplot \
                dictionary, which contains the postion of the subplot (and \
                whether to share the x-axis and/or the y-axis.)")
            if ('sharex' in subplot) and ('sharey' in subplot):
                sharex_ax = axes[int(subplot['sharex'])-1]
                sharey_ax = axes[int(subplot['sharey'])-1]
                axes.append(pyplot.subplot(pos, sharex = sharex_ax,
                                           sharey = sharey_ax))
            elif 'sharex' in subplot:
                sharex_ax = axes[int(subplot['sharex'])-1]
                axes.append(pyplot.subplot(pos, sharex = sharex_ax))
            elif 'sharey' in subplot:
                sharey_ax = axes[int(subplot['sharey'])-1]
                axes.append(pyplot.subplot(pos, sharey = sharey_ax))
        elif type(subplot) == int:
            axes.append(pyplot.subplot(subplot))
        else:
            raise ValueError("The subplot varaible for a graph must be either \
            a 3-digit integer or a list, which contains: the postion of the \
            subplot (3-digit integer) and whether to share the x-axis (integer \
            of graph to share axis with) and/or the y-axis (integer of graph \
            to share axis with).")
